function setupTraceSearcher(server, linksStr) {
  ...
  function getSearcherHtml() {
    ...
    if (has('yahoo', 'amazon')) { 
      searcherHtml += '<div id="trace-searcher-search-title" class="seesaaNazoruSearch">åüçıÇ∑ÇÈ</div>';
      searcherHtml += '<ul id="trace-searcher-search-list" class="seesaaNazoruSearchList">';
      if (has('yahoo')) { searcherHtml += '<li class="seesaaNazoruSearchYahoo"><a id="floating-yahoo-keyword" target="_blank">Yahoo!Ç≈åüçı</a></li>'; }
      if (has('amazon')) { searcherHtml += '<li class="seesaaNazoruSearchAmazon"><a id="floating-amazon-keyword" target="_blank">AmazonÇ≈åüçı</a></li>'; }
      searcherHtml += '</ul>';
    }
    if (has('twitter', 'blog')) { 
      searcherHtml += '<div class="seesaaNazoruPost">ìäçeÇ∑ÇÈ</div><ul class="seesaaNazoruPostList">'; 
      if (has('twitter')) { searcherHtml += '<li id="floating-twitter-kyeword-item" class="seesaaNazoruPostTwitter"><a id="floating-twitter-keyword" target="_blank">TwitterÇ…ìäçe</a></li>'; }
      if (has('blog')) { searcherHtml += '<li class="seesaaNazoruPostBlog"><a id="floating-blog-keyword" target="_blank">SeesaaÉuÉçÉOÇ…ìäçe</a></li>'; }
      searcherHtml += '</ul>';
    }
    searcherHtml += '</div>';
    searcherHtml += '</div>';
  ...
  function getSelectedText() {
    if (document.getSelection) { // Firefox
      ...
      //return document.innerText.substring(document.selectionStart, document.selectionEnd);
  ...
  observe(document.body, 'mouseup', function(evt) {
    ...
    function getLink(type, selection) {
      if (type == 'yahoo') {
        ...
        return 'http://match.seesaa.jp/ot_listing.pl?sid=trave_searcher&hid=81&c=12&tid=seesaa_hotspot&search=1&k=' +
        ...
        return 'http://twitter.com/?status=' + 
          encodeURIComponent('Åu' + selection + 'Åv ' + window.location);
      }
      else if (type == 'blog') {
