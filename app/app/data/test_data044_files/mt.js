...
function mtScore(entry_id) {
    ...
    var url = 'http://www.amagase.jp/cms/mt-cp.cgi';
...
function script_follow(id) {
    ...
    var xh = mtGetXmlHttp();
    if (!xh) return false;
    xh.open('POST', 'http://www.amagase.jp/cms/mt-cp.cgi', true);
    xh.onreadystatechange = function() {
        if ( xh.readyState == 4 ) {
            if (xh.status && ( xh.status != 200 ) ) {
                // error - ignore
    ...
    if (span) span.innerHTML = '<img src="http://www.amagase.jp/cms/mt-static/images/indicator.white.gif" height="10" width="10" alt="Following..." />';
}
...
function script_leave(id) {
    ...
    var span = document.getElementById('following-status');
    if (span) span.innerHTML = '<img src="http://www.amagase.jp/cms/mt-static/images/indicator.white.gif" height="10" width="10" alt="Leaving..." />';
}
...
var mtCookieDomain = ".amagase.jp";
var mtCookiePath = "/blog/";
...
function mtFetchUser(cb) {
    ...
    if ( ( cb == 'mtSetUser' ) && mtGetUser() ) {
        ...
        script.src = 'http://www.amagase.jp/cms/mt-comments.cgi?__mode=session_js&blog_id=1&jsonp=' + cb + '&ts=' + ts;
...
function mtCommentOnSubmit(f) {
    if (!mtRequestSubmitted) {
        ...
        if (f.armor)
            f.armor.value = '742b6ca8ec8c1350d36c279669765ef28016ad61';
...
function mtSignIn() {
    ...
    var url = 'http://www.amagase.jp/cms/mt-cp.cgi?__mode=login&blog_id=1';
    if (is_preview) {
        if ( document['comments_form'] ) {
            var entry_id = document['comments_form'].entry_id.value;
            url += '&entry_id=' + entry_id;
        } else {
            url += '&return_url=http%3A%2F%2Fwww.amagase.jp%2Fblog%2F';
        }
    } else {
...
function mtSignOut(entry_id) {
    ...
    var doc_url = document.URL;
    doc_url = doc_url.replace(/#.+/, '');
    var url = 'http://www.amagase.jp/cms/mt-cp.cgi?__mode=logout&blog_id=1';
    if (is_preview) {
        if ( document['comments_form'] ) {
            var entry_id = document['comments_form'].entry_id.value;
            url += '&entry_id=' + entry_id;
...
function mtShowGreeting() {
    ...
    if ( u && u.is_authenticated ) {
        if ( u.is_banned ) {
            ...
            var user_link;
            if ( u.is_author ) {
                user_link = '<a href="http://www.amagase.jp/cms/mt-cp.cgi?__mode=edit&amp;return_to=' + encodeURIComponent(document.URL) + '&amp;blog_id=1';
                user_link += '">' + u.name + '</a>';
            } else {
