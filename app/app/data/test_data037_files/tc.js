...
$Id: tc.js 68404 2007-12-03 01:27:05Z hsakai $
*/
...
TC.filterElementsByClassName = function( elements, className )
{
	if( !elements )
		return [];
	if( !TC.defined( className ) || className == null || className == "" )
		return elements;
	var filtered = [];
	for( var i = 0; i < elements.length; i++ )
	{
...
TC.filterElementsByTagName = function( elements, tagName )
{
	if( !elements || !tagName )
		return [];
	if( tagName == "*" )
...
TC.getElementsByTagAndClassName = function( tagName, className, root )
{
	...
	return TC.filterElementsByClassName( elements, className );
}
...
TC.getAncestors = function( element )
{
	if( !element )
		return [];
	var ancestors = [];
...
TC.getAncestorsByTagName = function( element, tagName )
{
	var elements = TC.getAncestors( element );
	return TC.filterElementsByTagName( elements, tagName );
}


TC.getFirstAncestorByTagName = function( element, tagName )
{
	return TC.getAncestorsByTagName( element, tagName )[ 0 ];
}


TC.getAncestorsByClassName = function( element, className )
{
	var elements = TC.getAncestors( element );
	return TC.filterElementsByClassName( elements, className );
}
...
TC.getFirstAncestorByClassName = function( element, className )
{
	return TC.getAncestorsByClassName( element, className )[ 0 ];
}


TC.getAncestorsByTagAndClassName = function( element, tagName, className )
{
	var elements = TC.getAncestorsByTagName( element );
	return TC.filterElementsByClassName( elements, className );
}


TC.getFirstAncestorByTagAndClassName = function( element, tagName )
{
	return TC.getAncestorsByTagAndClassName( element, tagName, className )[ 0 ];
}
...
TC.isTagNameLower = function( element, tagName )
{
	if( element.tagName && (tagName == "*" || element.tagName.toLowerCase() == tagName) )
		return true;
	return false;
}


TC.isTagName = function( element, tagName )
{
	return TC.isTagNameLower( element, tagName.toLowerCase() );
}
...
TC.getPosition = function( element )
{
	element = TC.elementOrId( element );
...
{
	var pos = TC.getPosition( element );
